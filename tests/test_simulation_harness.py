...
def find_crossover(selections: List[str], window_size: int = 200, min_stable_steps: int = 100) -> int:
    """
...
def run_beta_binomial_simulation(decay_factor: float, num_iterations: int = 10000) -> Tuple[int, List[str], List[float]]:
    ...
    for t in range(num_iterations):
        ...
        chosen_candidate, trace_id = router.route_with_trace("simulated_context", candidates)
...
def run_linear_simulation(
    ...
    for t in range(num_iterations):
        ...
        theta_stars = {
            arm: (c_vals[arm] / mu_norm_sq) * mu for arm in candidates
        }
...
def test_beta_binomial_adaptation():
    ...
    assert results[1.0] == -1 or results[1.0] > 800, f"Decay factor 1.0 adapted unexpectedly fast: {results[1.0]} steps"
    
    # With decay_factor = 0.99, effective window is ~100 steps. It should adapt quickly.
    assert 0 < results[0.99] < 500, f"Decay factor 0.99 adaptation took too long: {results[0.99]} steps"
    
    # With decay_factor = 0.95, effective window is ~20 steps. It should adapt extremely quickly.
    assert 0 < results[0.95] < 300, f"Decay factor 0.95 adaptation took too long: {results[0.95]} steps"

    # Verify that decaying models adapted strictly faster than the non-decaying model (decay_factor = 1.0)
    if results[1.0] != -1:
        assert results[0.99] < results[1.0], f"Decay factor 0.99 ({results[0.99]}) was not faster than 1.0 ({results[1.0]})"
        assert results[0.95] < results[1.0], f"Decay factor 0.95 ({results[0.95]}) was not faster than 1.0 ({results[1.0]})"
...
def test_linear_bandits_stability_and_drift():
    ...
    for mode in ["lints", "linucb"]:
        ...
        print(f"Mode: {mode} (Diagonal Covariance) -> Crossover at {crossover} steps after drift")
        
        # Verify adaptation
        assert 0 < crossover < 1500, f"Linear mode {mode} failed to adapt timely: {crossover} steps"
        ...
        for candidate, (precision, reward_vector) in final_params.items():
            ...
            assert np.all(np.isfinite(precision)), f"NaN/Inf precision found for {candidate} in {mode}"
            assert np.all(np.isfinite(reward_vector)), f"NaN/Inf reward vector found for {candidate} in {mode}"
            
            # Ensure diagonal precision elements do not diverge or drop below lambda (1.0)
            assert np.all(precision >= 1.0), f"Precision dropped below lambda for {candidate} in {mode}"
            ...
            assert np.all(precision <= 500.0), f"Precision diverged/blew up for {candidate} in {mode}: {precision}"
...
if __name__ == "__main__":
    ...
    print("-" * 65)
    print(f"{'Decay Factor':<15} | {'Crossover (Steps)':<20} | {'Adaptation Status':<20}")
    print("-" * 65)
    for df in [1.0, 0.99, 0.95, 0.90]:
        crossover, _, _ = run_beta_binomial_simulation(decay_factor=df)
        status = "NO ADAPTATION" if crossover == -1 else f"SUCCESS (< {crossover} steps)"
    ...
    print("-" * 80)
    print(f"{'Mode':<10} | {'Covariance':<10} | {'Crossover (Steps)':<20} | {'Min Precision':<15} | {'Max Precision':<15}")
    print("-" * 80)
    ...
    for mode in ["lints", "linucb"]:
        for diag in [True, False]:
            ...
            print(f"{mode:<10} | {cov_type:<10} | {str(crossover):<20} | {all_prec_min:<15.4f} | {all_prec_max:<15.4f}")
