...
def test_hybrid_initialization_validation():
    ...
    with pytest.raises(
        ValueError, match="Hybrid mode is only supported with linear bandit modes"
    ):
...
def test_hybrid_generalization_cold_start(mem_storage, det_embedder):
    ...
    for _ in range(20):
        candidate_name, trace_id = router.route_with_trace(
            "solve math equation", candidate_train
        )
        reward = 1.0 if candidate_name == "tool_math" else 0.0
        router.feedback_by_trace(trace_id, reward=reward)
...
def test_hybrid_metadata_string_embedding(mem_storage, det_embedder):
    ...
    candidate_name, trace_id = router.route_with_trace(
        "do calculus equations", ["tool_math", "tool_other"]
    )
...
async def test_async_hybrid_routing_and_feedback(async_sqlite_storage, det_embedder):
    ...
    candidate_name, trace_id = await router.aroute_with_trace(
        "do math algebra", candidates
    )
    await router.afeedback_by_trace(trace_id, reward=1.0)

    # Retrieve async beliefs
    mean_val, uncertainty_val = await router.aget_candidate_beliefs(
        "do math algebra", "tool_math"
    )
    assert mean_val > 0.5
