...
def test_check_citation_default_patterns():
    ...
    assert check_citation("No information available on parental leave rollover.") is False
    ...
    assert check_citation("Our policy on parental leave rollover allows up to 2 weeks to be rolled over.") is True
...
def test_check_citation_custom_patterns():
    ...
    assert check_citation("Sorry, this is a custom fallback phrase.", custom_patterns) is False
    assert check_citation("System says access denied.", custom_patterns) is False
    assert check_citation("I couldn't find it", custom_patterns) is True  # Default pattern ignored
...
def test_evaluate_rag_success():
    ...
    # Passes both checks
    assert evaluate_rag_success("Employees get 4 weeks of vacation.", source, faithfulness_threshold=0.5) is True
    
    # Fails citation check (matches fallback)
    assert evaluate_rag_success("I'm sorry, I couldn't find vacation info.", source) is False
...
def test_evaluate_rag_success_custom_parameters():
    ...
    assert evaluate_rag_success(
        response=response,
        source_chunks=source,
        faithfulness_threshold=0.1,
        fallback_patterns=[r"not found"],
        stop_words=set()
    ) is False  # Delta, Epsilon not in Alpha Beta Gamma -> 0.0 < 0.1
...
def test_process_ui_feedback_sync():
    ...
    # String thumbs up
    for thumbs_up in ["thumbs_up", "thumbs-up", "ThumbsUp", "like", "upvote", "success", "yes", "true", "1"]:
    ...
    # String thumbs down
    for thumbs_down in ["thumbs_down", "thumbs-down", "ThumbsDown", "dislike", "downvote", "failure", "no", "false", "0"]:
    ...
    with pytest.raises(ValueError, match="Numerical UI feedback must be between 0.0 and 1.0"):
